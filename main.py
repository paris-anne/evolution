...
# enviro.add_antibiotics() #concentration, time between dose(s) , halflife of dose
...
def run(first_dose, anti_conc, anti_freq, anti_halflife, skipped_doses,double_doses, numberofdoses, numberofagents): #first dose, anti_conc,
	...
	enviro.add_agents(number_of_agents=numberofagents)
	data = enviro.display(200000)
...
run(first_dose = 50000, anti_conc = 0.2, anti_freq = 10000, anti_halflife = 4400, skipped_doses = [] , double_doses = [], numberofdoses = 10, numberofagents = 2) #first dose, anti_conc,
#changing_starttime()
