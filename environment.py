...
class Environment(object):
	...
	def add_antibiotics(self, first_dose=0, anti_conc=0.1, anti_freq=4300, anti_halflife=1400, skipped_doses=[], double_dose=False, double_doses=[], numberofdoses=10):
		self.set_first_dose(first_dose)
		self.set_anti_conc(anti_conc)
		self.set_anti_freq(anti_freq)
		self.set_anti_halflife(anti_halflife)	
		self.set_skipped_doses(skipped_doses)	
		self.set_double_doses(double_doses)
		self.set_numberofdoses(numberofdoses)
		amount = 0
	...
	def add_agents(self, number_of_agents = 10, size = 3.0):
		for i in range(number_of_agents):
			...
			agent = ag.Agent(x=x, y=y, environment=self, size = size, reproduction = reproduction, 
	...
	def display(self, time):
		...
		while running:
			...
			data2[self.time_ms]=list(self.agents.values())
			...
			for i in self.agents: 
				...
				if self.agents[i].speed != 0:
					...
					if self.agents[i].food_level < 0.01:
		...
		resistant_data = pd.concat(dataframes, axis=1)
		total_data=pd.concat(dataframes2, axis=1)

		pop=pd.DataFrame({"population" :total_data.count()})
		#print(data)
		#print(pop)
		pl.figure(1)
		...
		return [resistant_data, total_data]
