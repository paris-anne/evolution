...
class Environment(object):
    def __init__(self, width = 100, height = 100, colour = (255,255,255)):
        ...
        self.anti_freq = 0
        self.anti_conc = []
        self.tbirths = 100000000000000
        self.time_ms = 0
        self.antibiotics_count = 0

        self.immune_system = 1100
        self.killfrac = 0
    ...
    def add_agents(self, number_of_agents = 10, size = 3.0):
        ...
        for i in range(number_of_agents):
            ...
            if agent.resistance == 1:
                agent.colour = (0,0,255)
            self.agents[agent.key] = agent
        ...
        while running:
            ...
            if self.antibiotics:
                ...
                for i in self.antibiotics:
                    ...
                    if i.effectiveness < 1.2*(np.e)**(-(self.anti_freq)/self.anti_halflife):
            ...
            if self.antibiotics_count < self.numberofdoses:
                if self.time_ms == 0:
                    ...
                    if 0 not in self.skipped_doses:
                        if 0 in self.double_doses:   #covers intial dose
                ...
                if self.time_ms-self.tbirths >self.anti_freq:
                    ...
                    if self.antibiotics_count not in self.skipped_doses:
                        ...
                        print(self.antibiotics_count, self.double_doses)
            ...
            if self.time_ms%self.immune_system == 0:
                ...
                if len(self.agents) < 250:
                    kill_number = killfrac*(len(self.agents))
                else:
                    kill_number = killfrac * 250

                for i in range(math.ceil(kill_number)):
                    if self.agents:
            ...
            for i in self.antibiotics:
                if i.colour[0]<254 and i.colour[1]<254 and i.colour[2]<254:
                    i.colour = np.add(i.colour, (2,1,2))
